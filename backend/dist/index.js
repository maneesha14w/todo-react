...
app.post("/todos", async (req, res) => {
    try {
        ...
        const newTodo = await pool.query("INSERT INTO todo (todo_title) VALUES ($1) RETURNING *", [todo_title]);
...
// toggle isComplete
app.put("/toggleIsComplete/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const updateTodo = await pool.query("UPDATE todo SET is_complete = NOT is_complete WHERE todo_id = $1", [id]);
